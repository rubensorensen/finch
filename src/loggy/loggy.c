...
static int32_t determine_color_support()
{
    int fd[2];
    if (pipe(fd) < 0) {
        LG_FATAL(&logger, "Could not make pipe: %s\n", strerror(errno));
    };

    pid_t pid = fork();
    if (pid < 0) {
        LG_FATAL(&logger, "Could not fork process: %s\n", strerror(errno));
    }
    ...
    if (pid == 0) {
        ...
        if (dup2(fd[1], STDOUT_FILENO) < 0) {
            LG_FATAL(&logger, "Could not duplicate stdout file descriptor: %s\n", strerror(errno));
        }
        close(fd[1]);
        if (execl("/bin/tput", "/bin/tput", "colors", NULL) < 0) {
            LG_FATAL(&logger, "Could not exec tput: %s\n", strerror(errno));
        }
    }
...
static void format_msg(char* dest, LgLogger* logger, LgLevel level,
        ...
        switch (fmt[i]) {
            ...
            case 'F': {
                ...
                switch (thing) {
                    ...
                    default: {
                        LG_ERROR(logger, "Unrecognized format option '%%%c'", thing);
                    }
                }
            } break;
...
void loggy_log(LgLogger* logger, LgLevel level, const char* file, int line, const char* fmt, ...)
{
    char buf[1000] = {0};
